    ...
    public class XMLProcessorLINQ
    {
            ...
            foreach (var item in propertiesElements)
            {
                switch(item.Name.ToString())
                {
                    ...
                    case "nation":
                        ...
                        break;
                    case "rating":
                        tank_platoon.rating = Int32.Parse(item.Value);
                        break;
                    case "win_rate":
                        tank_platoon.win_rate = float.Parse(item.Value);
                        break;
                }
            }
        }
        ...
        private void SetTropheys()
        {
            ...
            foreach(var item in tropheys)
            {
                ...
                trophey.year = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_YEAR).Value);
                trophey.place = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_PLACE).Value);
        ...
        private void SetPlayers()
        {
            ...
            foreach(var item in players)
            {
                ...
                player.age = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_AGE).Value);
                player.country = item.Element(TankPlatoonElements.TPLATOON_COUNTRY).Value;
                player.day = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_DATE_OF_BIRTH)
                    .Element(TankPlatoonElements.TPLATOON_DAY).Value);
                player.month = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_DATE_OF_BIRTH)
                    .Element(TankPlatoonElements.TPLATOON_MONTH).Value);
                player.birth_year = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_DATE_OF_BIRTH)
                    .Element(TankPlatoonElements.TPLATOON_BIRTH_YEAR).Value);
                player.personal_win_rate = float.Parse(item.Element(TankPlatoonElements.TPLATOON_PERS_WIN_RATE).Value);
                ...
                player.Tanks.Guns.gun_penetration = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_TANK)
                    .Element(TankPlatoonElements.TPLATOON_GUN_PENETRATION).Value);
                player.Tanks.top_speed = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_TANK)
                ...
                player.Tanks.tier = Int32.Parse(item.Element(TankPlatoonElements.TPLATOON_TANK)
